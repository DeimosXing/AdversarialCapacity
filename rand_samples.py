...
for param in model.parameters():
    ...
    param.data = param.data / torch.sum(param.data)
...
x_var = x.to(device)
# print(type(x_var[0][0]))
x_var = torch.rand(784, dataset_size).to(device).float()
...
for _ in range(iter_times):
    ...
    print("max:   ", max(noise.reshape(-1)))
    print("min:   ", min(noise.reshape(-1)))
    ...
    if (_ + 1) % 10 == 0:
        ...
        print(_ + 1)
        tmp = noise.cpu().detach().numpy().reshape(28, 28)
