...
def train(model, train_loader, optimizer, criterion, adversary, iter_):
    ...
    for batch_idx, (imgs, targets) in enumerate(train_loader):
        ...
        if iter_+1 > args.delay:
            ...
            y_pred = pred_batch(imgs, model)
            data_grad = imgs.grad

            data_grad = data_grad.data
            x_adv = fgsm_attack(imgs, 0.3, data_grad)
...
#                     best_adv_accuracy = adv_acc
#                     utils.checkdir(f"{os.getcwd()}/saves/{args.arch_type}/{args.dataset}/")
#                     torch.save(model,f"{os.getcwd()}/saves/{args.arch_type}/{args.dataset}/{_ite}_adv_model_{args.prune_type}.pth")
#
#             all_loss[iter_] = loss
#             all_accuracy[iter_] = accuracy
#             all_adv_accuracy[iter_] = adv_acc
...
#         plt.grid(color="gray")
#         utils.checkdir(f"{os.getcwd()}/plots/lt/standard/{args.arch_type}/{args.dataset}/")
#         plt.savefig(f"{os.getcwd()}/plots/lt/standard/{args.arch_type}/{args.dataset}/{args.prune_type}_LossVsStandardAccuracy_{comp1}.png", dpi=1200)
#         plt.close()
#
#         # Plotting Loss (Training), Accuracy (Testing), Iteration Curve
#         #NOTE Loss is computed for every iteration while Accuracy is computed only for every {args.valid_freq} iterations. Therefore Accuracy saved is constant during the uncomputed iterations.
...
#         plt.grid(color="gray")
#         utils.checkdir(f"{os.getcwd()}/plots/lt/adversarial/{args.arch_type}/{args.dataset}/")
#         plt.savefig(f"{os.getcwd()}/plots/lt/adversarial/{args.arch_type}/{args.dataset}/{args.prune_type}_LossVsAdversarialAccuracy_{comp1}.png", dpi=1200)
#         plt.close()
#
#         # Dump Plot values
#         utils.checkdir(f"{os.getcwd()}/dumps/lt/{args.arch_type}/{args.dataset}/")
...
#     plt.grid(color="gray")
#     utils.checkdir(f"{os.getcwd()}/plots/lt/standard/{args.arch_type}/{args.dataset}/")
#     plt.savefig(f"{os.getcwd()}/plots/lt/standard/{args.arch_type}/{args.dataset}/{args.prune_type}_StandardAccuracyVsWeights.png", dpi=1200)
#     plt.close()
#
#     plt.plot(a, bestadvacc, c="blue", label="Winning tickets")
#     plt.title(f"Test Adversarial Accuracy vs Unpruned Weights Percentage ({args.dataset},{args.arch_type})")
...
#     plt.grid(color="gray")
#     utils.checkdir(f"{os.getcwd()}/plots/lt/adversarial/{args.arch_type}/{args.dataset}/")
#     plt.savefig(f"{os.getcwd()}/plots/lt/adversarial/{args.arch_type}/{args.dataset}/{args.prune_type}_AdversarialAccuracyVsWeights.png", dpi=1200)
#     plt.close()
#
# # Function for Training
# def train(model, train_loader, optimizer, criterion, adversary, iter_):
...
#         x_var, y_var = to_var(imgs), to_var(targets.long())
#         imgs, targets = imgs.to(device), targets.to(device)
#         train_loss = criterion(model(x_var), y_var)
#         # adversarial training
#         if iter_/+1 > args.delay:
#             # use predicted label to prevent label leaking
#             y_pred = pred_batch(imgs, model)
#             x_adv = adv_train(imgs, y_pred, model, criterion, adversary)
#             x_adv_var = to_var(x_adv)
#             loss_adv = criterion(model(x_adv_var), y_var)
#             train_loss = (train_loss + loss_adv) / 2
#         if (batch_idx + 1) % 100 == 0:
