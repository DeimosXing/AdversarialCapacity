...
class FGSMAttack(object):
    ...
    def perturb(self, X_nat, y, epsilons=None, model=None):
        """
        ...
        loss = to_var(loss, requires_grad = True)
        loss.backward()
        if to_var(X_var).grad is not None:
            grad_sign = to_var(X_var).grad.data.cpu().sign().numpy()
